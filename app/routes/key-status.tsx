...
async function fetchKeyStatus(key: string) {
  ...
  const isDemo = key.startsWith("sk-ant-api03-") || key === "demo" || key.toLowerCase().includes("placeholder") || key.toLowerCase().includes("demo");

  try {
    const data = await getKeyStatus(key);
...
export default function KeyStatusRoute() {
 ...
 <button
 type="submit"
 className="inline-flex items-center justify-center gap-2 rounded-xl text-sm font-semibold bg-primary text-primary-foreground hover:bg-primary/95 transition-all px-6 h-12 cursor-pointer shadow-md shadow-primary/20 hover:scale-[1.01]"
 >
 <svg
 xmlns="http://www.w3.org/2000/svg"
 width={24}
 ...
 {keyData && (keyData as any).mockWarning && (
