...
interface Plan {
    ...
    secondary_price_text?: string | null;
...
function PlanSheet({ isOpen, plan, mode, onClose, onSave, saving }: PlanSheetProps) {
    ...
    const handleSave = () => {
        ...
        onSave({
            ...
            secondary_price_text: form.secondary_price_text.trim() || null,
            button_text: form.button_text.trim() || "Get this plan →",
            button_subtext: form.button_subtext.trim() || "Instant key delivery after payment",
            is_dark_card: form.is_dark_card,
        ...
        <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
            <DialogContent className="w-full max-w-2xl md:max-w-3xl max-h-[90vh] overflow-y-auto p-6 sm:p-7 rounded-2xl sm:rounded-3xl border border-border bg-background shadow-2xl">
                ...
                >
                        ...
                        <div className="flex flex-wrap gap-1.5 mt-2">
                            {PRESET_DURATIONS.map((preset) => (
                                <button
                                    ...
                                    className={`px-2.5 py-1 rounded-lg text-[11px] font-medium transition-all cursor-pointer border ${form.duration_days === String(preset.value)
                    ...
                    {/* ── OpusLive Theme & Card Box Styling Options ── */}
                    <div className="p-4 rounded-xl border border-orange-500/30 bg-orange-500/[0.04] space-y-3.5">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            <div>
                                ...
                                <Input
                                    placeholder="e.g. TRY IT OUT or BEST VALUE"
                                    {...field("badge_text")}
                                />
                                <p className="text-[10px] text-muted-foreground mt-1">
                                    Floating top badge pill text
                                </p>
                            </div>
                            <div>
                                <label className="block text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-1.5">
                        ...
                        <div>
                            <label className="block text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-1.5">
                                Secondary Price Tagline
                            </label>
                            <Input
                                placeholder="e.g. ≈₹3,000 • 5× Claude Pro"
                                {...field("secondary_price_text")}
                            />
                            <p className="text-[10px] text-muted-foreground mt-1">
                                Sub-text rendered right below the main price
                            </p>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            <div>
                                ...
                                <Input
                                    placeholder="Get this plan →"
                                    {...field("button_text")}
                                />
                            </div>
                            <div>
                                <label className="block text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-1.5">
                                    Button Micro-Subtext
                                </label>
                                <Input
                                    placeholder="Instant key delivery after payment"
                                    {...field("button_subtext")}
                                />
                            </div>
                        </div>

                        {/* Dark Theme Card Toggle */}
                        <div className="pt-2 border-t border-orange-500/20 flex items-center justify-between">
                            <div>
                                <p className="text-xs font-semibold text-foreground flex items-center gap-1.5">
                                    🌙 Dark Featured Theme Box
                                </p>
                                <p className="text-[11px] text-muted-foreground">
                                    Render card with black background & solid orange CTA button (like 20x Plan)
                                </p>
                            </div>
                            <button
                                type="button"
                                onClick={() => setForm((f) => ({ ...f, is_dark_card: !f.is_dark_card }))}
                                className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors cursor-pointer focus:outline-none ${form.is_dark_card ? "bg-orange-600" : "bg-muted"
                                    }`}
                                role="switch"
                                aria-checked={form.is_dark_card}
                            >
                                <span
                                    className={`inline-block h-4 w-4 transform rounded-full bg-white shadow transition-transform ${form.is_dark_card ? "translate-x-[22px]" : "translate-x-1"
                                        }`}
                                />
                            </button>
                        </div>
                        ...
                        <div className="flex items-center justify-between">
                            <div>
                                ...
                                <p className="text-[11px] text-muted-foreground">
                                    Emphasizes plan borders and badges
                                </p>
                            </div>
                            <button
                                type="button"
                                onClick={() => setForm((f) => ({ ...f, is_popular: !f.is_popular }))}
                                className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors cursor-pointer focus:outline-none ${form.is_popular ? "bg-amber-500" : "bg-muted"
                                    }`}
                                role="switch"
                                aria-checked={form.is_popular}
                            >
                                <span
                                    className={`inline-block h-4 w-4 transform rounded-full bg-white shadow transition-transform ${form.is_popular ? "translate-x-[22px]" : "translate-x-1"
                                        }`}
                                />
                            </button>
                        </div>
                    ...
                    <div className="flex items-center justify-between p-3 rounded-xl border border-border bg-muted/20">
                        ...
                        <button
                            ...
                            className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors cursor-pointer focus:outline-none focus:ring-2 focus:ring-primary/50 ${form.is_active ? "bg-primary" : "bg-muted"
                                }`}
                            role="switch"
                            aria-checked={form.is_active}
                        >
                            <span
                                className={`inline-block h-4 w-4 transform rounded-full bg-white shadow transition-transform ${form.is_active ? "translate-x-[22px]" : "translate-x-1"
                                    }`}
                            />
                        </button>
                    </div>
                ...
                <DialogFooter className="sticky bottom-0 z-20 mt-6 pt-4 pb-1 border-t border-border/80 bg-background/95 backdrop-blur-md flex flex-row justify-end gap-3">
                    <Button
                        variant="outline"
                        onClick={onClose}
                        disabled={saving}
                        className="px-5 py-2.5 h-10 rounded-xl"
                    >
                        Cancel
                    </Button>
                    <Button
    ...
    const handleSave = useCallback(
        async (formData: Partial<Plan>) => {
            ...
            const payload = {
                ...
                secondary_price_text: formData.secondary_price_text ?? null,
                button_text: formData.button_text || "Get this plan →",
                button_subtext: formData.button_subtext || "Instant key delivery after payment",
                is_dark_card: formData.is_dark_card ?? false,
                price_usdt: formData.price_usdt ?? 0,
                            ...
                            <AnimatePresence mode="popLayout">
                                {paginated.map((plan, index) => (
                                    <motion.div
                                        ...
                                        className={`group relative rounded-2xl border overflow-hidden bg-card dark:bg-card/60 transition-all hover:shadow-lg hover:shadow-primary/5 ${plan.is_active ? "border-border hover:border-primary/30" : "border-border opacity-60"
                                            }`}
                                    >
                                        {/* Card Header */}
                                        <div className={`p-5 bg-gradient-to-br ${PLAN_COLORS[index % PLAN_COLORS.length]} relative overflow-hidden`}>
                                            ...
                                            <div className="flex items-start justify-between">
                                                ...
                                                <div className="flex items-center gap-1 flex-wrap justify-end">
                                                    {plan.badge_text && (
                                                        <span className="text-[10px] font-extrabold px-2 py-0.5 rounded-full bg-orange-500 text-white uppercase tracking-wider shadow-xs">
                                                            {plan.badge_text}
                                                        </span>
                                                    )}
                                                    {plan.is_dark_card && (
                                                        <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full bg-neutral-900 text-orange-400 border border-orange-500/30">
                                                            🌙 DARK BOX
                                                        </span>
                                                    )}
                                                    {!plan.is_active && (
                                            ...
                                            <div className="mt-4 flex items-end justify-between">
                                                <div>
                                                    <span className="text-2xl font-bold text-white">
                                                        {plan.price_usdt ? `$${plan.price_usdt} USDT` : formatCurrency(Number(plan.price), plan.currency)}
                                                    </span>
                                                    <span className="text-white/60 text-xs ml-1">
                                                        / {getDurationLabel(plan.duration_days)}
                                                    </span>
                                                    {plan.secondary_price_text && (
                                                        <p className="text-[11px] text-white/80 font-medium mt-0.5">{plan.secondary_price_text}</p>
                                                    )}
                                                </div>
                                                {plan.multiplier !== 1 && (
                ...
                <Dialog open={!!deleteId} onOpenChange={(open: boolean) => !open && setDeleteId(null)}>
                    <DialogContent className="w-full max-w-md p-6 rounded-2xl border border-red-500/20 bg-background shadow-2xl">
                        ...
                        <DialogFooter className="mt-6 flex flex-row justify-end gap-2.5">
                            ...
                            <Button
                                variant="destructive"
                                className="px-5 py-2 rounded-xl gap-1.5"
