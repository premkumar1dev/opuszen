...
export async function getDashboardStats(): Promise<{
	...
	// Dashboard stats don't need the api_key field — query only non-sensitive
	// columns to avoid the decryption overhead of getAllMasterKeys().
	const [
		masterKeysResult,
		userKeysResult,
		todayLogsResult,
		...
		supabase.from('master_api_keys').select('provider, health_status, status, remaining_credits', { count: 'exact', head: false }),
		...
		supabase.from('usage_statistics').select('provider, total_requests', { head: false }).gte('date', today),
	]);
	...
	for (const u of todayUsage) {
		const p = u.provider;
		if (providerMap.has(p)) {
			providerMap.get(p)!.requests += u.total_requests ?? 0;
		}
	}
