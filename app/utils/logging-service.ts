...
export async function logApiRequest(log: {
	...
	try {
		const bodyForLog = redactRequestBody(log.requestBody);
...
/** Redact sensitive fields from the request body before persisting */
function redactRequestBody(body: Record<string, any> | undefined): Record<string, any> {
	...
	for (const [key, value] of Object.entries(body)) {
		...
		if (["api_key", "key", "secret", "password", "token", "authorization", "auth"].some((s) => lower.includes(s))) {
			redacted[key] = "[redacted]";
		} else if (typeof value === "string" && value.length > 200) {
			// Truncate long strings (likely prompts/content)
			redacted[key] = value.slice(0, 200) + "… [truncated]";
		} else if (Array.isArray(value) && lower.includes("message")) {
			// messages array — redact content fields
			redacted[key] = (value as any[]).map((m: any) => ({
				role: m.role,
				content: m.content ? "[redacted]" : undefined,
			}));
		} else {
			redacted[key] = value;
		}
