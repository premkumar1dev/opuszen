...
export async function checkTokenWindow(
	...
	const totalTokensThisRequest = promptTokens + completionTokens;
	const allowed = used + totalTokensThisRequest <= windowTokensLimit;

	// Record this window's usage only if allowed (prevents overage recording)
	if (allowed) {
		await supabase.from("token_usage_windows").insert({
			...
			total_tokens: totalTokensThisRequest,
			request_count: 1,
		});
	}

	const tokensRemaining = Math.max(0, windowTokensLimit - used - (allowed ? totalTokensThisRequest : 0));

	return {
		allowed,
		tokensUsedInWindow: used + (allowed ? totalTokensThisRequest : 0),
		tokensRemaining,
